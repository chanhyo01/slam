...
### <span id="1.3">1.3. **livox_interfaces**</span>
Follow [livox_interfaces Installation](https://github.com/Livox-SDK/livox_interfaces).

You can also use the one I modified [livox_interfaces](https://github.com/Ericsii/livox_interfaces/tree/feature/use-standard-unit)
...
Connect to your PC to Livox LiDAR by following  [Livox-ros-driver2 installation](https://github.com/Livox-SDK/livox_interfaces), then
```bash
cd <ros2_ws>
. install/setup.bash # use setup.zsh if use zsh
...
```bash
ros2 launch livox_interfaces msg_MID360_launch.py
```

- For livox serials, FAST-LIO only support the data collected by the ``` livox_lidar_msg.launch ``` since only its ``` livox_interfaces/CustomMsg ``` data structure produces the timestamp of each LiDAR point which is very important for the motion undistortion. ``` livox_lidar.launch ``` can not produce it right now.
- If you want to change the frame rate, please modify the **publish_freq** parameter in the [livox_lidar_msg.launch](https://github.com/Livox-SDK/livox_ros_driver/blob/master/livox_interfaces/launch/livox_lidar_msg.launch) of [Livox-ros-driver](https://github.com/Livox-SDK/livox_interfaces) before make the livox_ros_driver pakage.
